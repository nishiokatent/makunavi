...
export default function TentsukuCanvas() {
  ...
  function drawFabricLabel(c:CanvasRenderingContext2D,showHandle:boolean,sc:number,ox:number,oy:number){
    ...
    roundRect(c,bx,by,fs*0.18,bh,fs*0.1);c.fill();
    ...
    c.fillText(line1,bx+fs*0.18+pad*0.7,by+pad*0.7);
    c.font=`400 ${fs*0.72}px ${serif}`;c.fillStyle='#6b5040';
    c.fillText(line2,bx+fs*0.18+pad*0.7,by+lh*0.85);
  ...
  async function shareImage(){
    ...
    out.toBlob(async blob=>{
      ...
      try{await navigator.share({files:[file],title:'てんつ君'});}
      catch{downloadImage();}
    },'image/png');
  }
