...
export default function DashboardPage() {
  ...
  return (
    <div className="px-4 sm:px-6 lg:px-7 py-5 sm:py-7 max-w-5xl mx-auto animate-fadeInUp">

      {/* ─── ウェルカムバナー ─────────────────────── */}
      <div
        className="relative rounded-2xl px-5 sm:px-7 py-5 sm:py-6 mb-5 sm:mb-7 overflow-hidden"
        style={{
      ...
      >
        ...
        />
        {/* 装飾テント — 狭い画面では非表示 */}
        <div className="hidden sm:block absolute right-6 top-1/2 -translate-y-1/2 opacity-10 pointer-events-none">
        ...
        <div className="relative">
          ...
          <h1
            className="text-lg sm:text-xl font-black text-white mb-1.5 sm:mb-2"
      ...
      {/* ─── クイック統計 ─────────────────────────── */}
      <div className="grid grid-cols-3 gap-2 sm:gap-4 mb-5 sm:mb-7 stagger">
        {[
          ...
          <div
            key={i}
            className="animate-fadeInUp bg-white rounded-xl sm:rounded-2xl px-3 sm:px-5 py-3 sm:py-4 border border-gray-100 shadow-sm"
          >
            <div className="text-[10px] sm:text-xs text-gray-500 mb-1 leading-tight">{stat.label}</div>
            <div className="text-lg sm:text-2xl font-black leading-tight" style={{ color: stat.color }}>
              {stat.value}
            </div>
            <div className="text-[9px] sm:text-[10px] text-gray-400 mt-0.5">{stat.sub}</div>
          </div>
        ))}
      </div>
      ...
      <section className="mb-6 sm:mb-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-3 sm:gap-4 stagger">
          {TOOLS.map(tool => (
        ...
        <section className="lg:col-span-2">
          ...
          <div className="bg-white border border-gray-100 rounded-2xl shadow-sm overflow-hidden">
            {RECENT_QUOTES.map((q, i) => (
              <div
                key={q.id}
                className={`flex items-center gap-3 px-3 sm:px-4 py-3 sm:py-3.5 cursor-pointer transition-colors hover:bg-gray-50 active:bg-gray-100 ${
              ...
              >
                ...
                <div className="text-sm font-black text-[#1A2F6E] whitespace-nowrap">
                  ¥{q.amount.toLocaleString()}
                </div>
              </div>
            ))}
            <div className="px-3 sm:px-4 py-3 border-t border-gray-50">
              <Link
                href="/tenmitsukun"
                className="text-xs text-[#1A2F6E] hover:underline font-medium flex items-center gap-1 active:opacity-60"
              >
                すべての見積もりを見る
...
function ToolCard({ tool }: { tool: typeof TOOLS[0] }) {
  const inner = (
    <div
      className="animate-fadeInUp relative rounded-2xl p-4 sm:p-5 transition-all duration-200 h-full cursor-pointer group overflow-hidden active:scale-[0.98]"
      style={{
        background: tool.bg,
    ...
    >
      ...
      <div
        className="w-11 h-11 rounded-xl flex items-center justify-center text-2xl mb-3 sm:mb-3.5 transition-transform duration-200 group-hover:scale-110"
      ...
      <p className="text-[11px] text-gray-500 leading-relaxed mb-3 line-clamp-3">
        {tool.desc}
      </p>
